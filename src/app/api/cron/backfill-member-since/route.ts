/**
 * One-off API Route: Backfill memberSince from HubSpot
 * Fetches date_joined_tio for all members that don't have memberSince set yet
 *
 * Usage: GET /api/cron/backfill-member-since
      ...
      try {
        ...
        if (dateJoined) {
          await prisma.member.update({
            where: { id: member.id },
            data: { memberSince: new Date(dateJoined) },
          });
        ...
        console.error(`Failed to backfill memberSince for ${member.name}:`, error);
